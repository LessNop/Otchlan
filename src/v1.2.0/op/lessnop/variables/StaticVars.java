...
public class StaticVars {
	...
	private void load() {
		...
		int i = add.stringToTime((String) Main.inst().getConfig().get("settings.czasOgolny")); 
		if(i < 1) Bukkit.getConsoleSender().sendMessage(mainPrefix + "§cBlad! Czas ogolny nie jest liczba dodatnia!");
		else this.time = i;
		
		i = add.stringToTime((String) Main.inst().getConfig().get("settings.czasWiadomosci")); 
		...
		i = add.stringToTime((String) Main.inst().getConfig().get("settings.czasOtwarcia")); 
